...
while not game.is_over():
    ...
    if game.current_player==1:  # we are assuming player 1 is a Human_Player
        ...
        for card in game.draw[game.current_player]:
            if(card == "Jw"):
                card = "jw"
            elif(card == "Jb"):
                card = "jb"

        if (game.wronganswers[game.current_player] != []):
            for wrongmove in game.wronganswers[game.current_player]:
                if wrongmove[0] == game.draw[game.current_player][-1]:
